...
def num_adj_symbols(start_row, start_col, row_stride, col_stride, player,board, width=BOARD_WIDTH, height=BOARD_HEIGHT):
    """ Starting at (start_row,start_col), check for the  number of consecutive adjacent symbols in a certain path
        of board entries around (start_row,start_col).  The count includes the symbol at (start_tow,start_col).

        ow_stride and col_stride determine the orientation of the path around (start_row, start_col).  For example,
        row_stride = -1 and col_stride = 1 results in a path up to and the right,
        beginning at (start_row + 3, start_col - 3).
    ...
    """
    count = 0
    if board[board_xy_to_ind(start_row,start_col)] == player:
        # Starting location has the player's symbol
        count += 1

    # dir determines the direction in which we search.  We start going in the "negative" striding direction (e.g.,
    # left if the path is horizontal), starting at (start_row, start_col).  Then we go in the "positive" striding
    # direction (e.g., right if the path is horizontal).
    for dir in [-1,1]:
        cur_row = start_row
        cur_col = start_col
        while(True):
            cur_row = cur_row + dir * row_stride
            cur_col = cur_col + dir * col_stride
            ind = board_xy_to_ind(cur_row,cur_col,width,height)
            if ind == -1 or board[ind] != player:
                # Either we passed the edge of the board or the current symbol is not player's symbol
                break

            # The player's symbol is at this spot.  Increment the count.
            count += 1

    # Search is complete.  Return the count.
    return count
...
def has_player_won(row, col, player, board, width=BOARD_WIDTH, height=BOARD_HEIGHT):
    ...
    if max_num_player_symbols(row,col,0,1,player,board,width,height) >= 4:
        return True

    # Check vertically (no col stride, but a row stride of 1)
    if max_num_player_symbols(row,col,1,0,player,board,width,height) >= 4:
        return True

    # Check diagonally down and to the right (row and col strides of 1)
    if max_num_player_symbols(row,col,1,1,player,board,width,height) >= 4:
        return True

    # Check diagonally up and to the right (row stride of -1 (to go up) and col stride of 1)
    return max_num_player_symbols(row,col,-1,1,player,board,width,height) >= 4

def max_consecutive_pieces(row, col, player, board, width=BOARD_WIDTH, height=BOARD_HEIGHT):
    """ Determine the maximum number of consecutive pieces of player that are adjacent to (row,col).
        Coordinates start in upper-left  hand corner of the board.
        ...
        Returns the maximum number of consecutive pieces (a non-negative integer).
    """

    l = [   num_adj_symbols(row,col,0,1,player,board,width,height),      #   Horizontally
            num_adj_symbols(row,col,1,0,player,board,width,height),      #   Vertically
            num_adj_symbols(row,col,1,1,player,board,width,height),      #   Diagonal and down
            num_adj_symbols(row,col,-1,1,player,board,width,height)      #   Diagonal and up
        ]
...
def find_move_off_two_pieces(player,num_filled_spaces_in_col, board, width = BOARD_WIDTH, height = BOARD_HEIGHT):
    # Determine if player's next move can add a piece to two consecutive pieces
    # Return -1 if there is no winning move available.
    empty_cols = [i for i, val in enumerate(num_filled_spaces_in_col) if val < BOARD_HEIGHT ]

    for col in empty_cols:
        test_row = height - num_filled_spaces_in_col[col] - 1
        if max_consecutive_pieces(test_row,col,player,board,width,height) >= 2:
            # Found a place where we can make three consecutive pieces
            return (test_row, col)
...
COMPUTER_MOVE_STEPS = [
    ...
    [find_winning_move, block_winning_move, find_move_off_two_pieces, drop_random_col_but_check_for_loss]
]
